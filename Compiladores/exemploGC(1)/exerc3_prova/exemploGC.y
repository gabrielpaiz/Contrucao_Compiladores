...
%token WHILE,TRUE, FALSE, IF, ELSE, FOR, BREAK, CONTINUE
...
cmd : exp ';'
    ...
    | WHILE {
					pRot.push(proxRot);  lLoop.push(proxRot); proxRot += 2; 
   ...
   | FOR'(' exp ';' { pRot.push(proxRot); lLoop.push(proxRot); proxRot += 4;
					System.out.printf("rot_%02d:\n",pRot.peek()+3);
			...
			exp')'{System.out.printf("\tJMP rot_%02d\n", (int)pRot.peek()+3);
				   System.out.printf("rot_%02d:\n",pRot.peek()+2);
				  } 
			
			cmd { System.out.printf("\tJMP rot_%02d   # terminou cmd na linha de cima\n", pRot.peek());
						System.out.printf("rot_%02d:\n",(int)pRot.peek()+1);
						pRot.pop();
						lLoop.pop();
						}
...
exp : ID '=' exp {  System.out.println("\tPOPL %EDX");
	...
	| ID INC 			{ gcExpInc($1, INC);}
	| ID DEC 			{ gcExpInc($1, DEC);}	
...
%%
  ...
  private Stack<Integer> lLoop = new Stack<Integer>();
  ...
  public static void main(String args[]) throws IOException {
	...
	public void gcExpInc(String ID, int op){
		switch(op){
			case Parser.INC:
